...
22. "secondary_network_nad" - (Optional) - The name of the secondary network for multi-homing and multi-networkpolicies tests. For tests except 27-31, the primary network will be used if unspecified (the default which is None). For mandatory tests 27-31 it defaults to "tft-secondary" if not set. Can be overridden per-node using the server/client level `secondary_network_nad` fields. The framework applies (idempotently) the auto-generated NAD when these test cases are selected: **OVN-Kubernetes overlay** (`ovn-k8s-cni-overlay`) for `net1`. When `resource_name` is set on the connection (or detected), TFT also sets the Multus standard **`k8s.v1.cni.cncf.io/resourceName` annotation on the NAD** (in addition to the OVN `resourceName` field inside `spec.config`) so device plugins and Multus can coordinate allocation with pod `resources` requests. Subnets, MTU, and topology default to `10.193.0.0/16/26`, `1500`, and `layer3`, overridable via `TFT_SECONDARY_NAD_SUBNETS`, `TFT_SECONDARY_NAD_MTU`, and `TFT_SECONDARY_NAD_TOPOLOGY`.
